...
// Lấy thông tin người dùng hiện tại từ hàm isLogin()
$userId = isLogin()['user_id'];  // Lấy user_id của người dùng hiện tại

// Lấy thông tin người dùng từ cơ sở dữ liệu
$userDetail = getUserInfo($userId);  // Giả sử hàm này trả về thông tin người dùng
$roomId = $userDetail['room_id'];    // Lấy room_id của người dùng (nếu cần)

// Truy vấn để lấy hợp đồng của người dùng dựa trên room_id
$contractDetail = firstRaw("SELECT * FROM contract WHERE room_id = $roomId");

// Kiểm tra xem hợp đồng có tồn tại không
if ($contractDetail) {
    $id = $contractDetail['id'];  // Lấy ID hợp đồng
} else {
    die("Không tìm thấy hợp đồng của người dùng.");
}

// Truy vấn để lấy thông tin tenant từ hợp đồng, sử dụng $id của hợp đồng
...
// Truy vấn thông tin dịch vụ của hợp đồng
$servicesDetail = firstRaw("SELECT services_id FROM contract_services WHERE contract_id = $id");
...
$contractDetail = firstRaw("SELECT 
    sotiencoc, 
    ngayvao, 
    ngayra, 
    dieukhoan1, 
